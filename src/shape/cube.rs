...
impl Cube {
	pub fn new(minimum: Point3, maximum: Point3, material: Rc<dyn Material>) -> Self {
		...
		sides.add(Rc::new(XyRect::new(
			minimum.x(), maximum.x(),
			minimum.y(), maximum.y(),
			maximum.z(), Rc::clone(&material),
		)));
		sides.add(Rc::new(XyRect::new(
			minimum.x(), maximum.x(),
			minimum.y(), maximum.y(),
			minimum.z(), Rc::clone(&material),
		)));

		sides.add(Rc::new(XzRect::new(
			minimum.x(), maximum.x(),
			minimum.z(), maximum.z(),
			maximum.y(), Rc::clone(&material),
		)));
		sides.add(Rc::new(XzRect::new(
			minimum.x(), maximum.x(),
			minimum.z(), maximum.z(),
			minimum.y(), Rc::clone(&material),
		)));

		sides.add(Rc::new(YzRect::new(
			minimum.y(), maximum.y(),
			minimum.z(), maximum.z(),
			maximum.x(), Rc::clone(&material),
		)));
		sides.add(Rc::new(YzRect::new(
			minimum.y(), maximum.y(),
			minimum.z(), maximum.z(),
			minimum.x(), Rc::clone(&material),
		)));
		Self {
			minimum,
			maximum,
